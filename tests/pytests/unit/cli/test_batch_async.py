...
def test_batch__del__(batch):
    ...
    batch.event = event
    batch.__del__()
    assert batch.local == None
    assert batch.event == None
    assert batch.ioloop == None
...
def test_batch_next(batch):
    ...
    batch.local.run_job_async.return_value = future
    batch.run_next()
    assert batch.local.run_job_async.call_args[0] == ({"foo", "bar"}, "my.fun", [], "list")
    assert batch.event.io_loop.spawn_callback.call_args[0] == (batch.find_job, {"foo", "bar"})
...
def test_batch_find_job(batch):
    ...
    assert batch.event.io_loop.spawn_callback.call_args[0] == (batch.check_find_job, {"foo", "bar"}, "1234")
...
def test_batch_find_job_with_done_minions(batch):
    ...
    salt.ext.tornado.gen.sleep = MagicMock(return_value=future)
    batch.find_job({"foo", "bar"})
    assert batch.event.io_loop.spawn_callback.call_args[0] == (batch.check_find_job, {"foo"}, "1234")
