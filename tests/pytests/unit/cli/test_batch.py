...
def test_gather_minions_with_batch_presence_ping(batch):
    ...
    with patch('salt.client.get_local_client', MagicMock(return_value=mock_client)):
        with patch('salt.client.LocalClient.cmd_iter', MagicMock(return_value=[])):
            Batch(opts_no_pp)
            Batch(opts_with_pp)
    assert mock_client.mock_calls[0][1][3] == opts_no_pp['timeout']
    assert mock_client.mock_calls[0][2]['gather_job_timeout'] == opts_no_pp['gather_job_timeout']
    assert mock_client.mock_calls[2][1][3] == opts_with_pp['batch_presence_ping_timeout']
    assert mock_client.mock_calls[2][2]['gather_job_timeout'] == opts_with_pp['batch_presence_ping_gather_job_timeout']
