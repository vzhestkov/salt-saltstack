...
def list_holds(pattern=None, full=True, **kwargs):
    ...
    for solv_name, lock in locks.items():
        if lock.get("type", "package") != "package":
            continue
        try:
            found_pkgs = search(solv_name, root=root, match=None if "*" in solv_name else "exact",
    ...
    for target in targets:
        ...
        if locks.get(target):
            ...
            if version and lock_ver != version:
                ...
                removed.append(target if not lock_ver else "{}={}".format(target, lock_ver))
...
def search(criteria, refresh=False, **kwargs):
    ...
    cmd.append(criteria)
    solvables = (
        __zypper__(root=root, ignore_no_matching_item=kwargs.get("ignore_no_matching_item", False))
