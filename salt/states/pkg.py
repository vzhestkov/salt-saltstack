        ...
        for lock in _locks:
            ...
            if match:
                ...
                if epoch == "0":
                    ...
                    epoch = "{}:".format(epoch)
                locks.update({match.group(1): {"version": "{}{}".format(epoch, match.group(3))}})
            else:
                locks.update({lock: {}})
        ...
        if pkg_name not in locks or (vr_lock and lock_ver != pkg_ver):
            if __opts__["test"]:
                if pkg_name in locks:
                    comments.append("The following package's hold rule would be updated: {}{}".format(
                        pkg_name, "" if not pkg_ver else " (version = {})".format(pkg_ver)))
                else:
                    comments.append("The following package would be held: {}{}".format(
                        pkg_name, "" if not pkg_ver else " (version = {})".format(pkg_ver)))
            else:
                unhold_ret = None
                if pkg_name in locks:
                ...
                if not hold_ret.get(pkg_name, {}).get("result", False):
                    ret["result"] = False
                if (unhold_ret and unhold_ret.get(pkg_name, {}).get("result", False) and
                        hold_ret and hold_ret.get(pkg_name, {}).get("result", False)):
                    comments.append("Package {} was updated with hold rule".format(pkg_name))
        ...
        for pkg_name in locks:
            ...
            if __opts__["test"]:
                if pkg_name not in held_pkgs:
                    comments.append("The following package would be unheld: {}".format(pkg_name))
            else:
                if pkg_name not in held_pkgs:
                    unhold_ret = __salt__["pkg.unhold"](name=name, pkgs=[pkg_name])
...
def unheld(name, version=None, pkgs=None, all=False, **kwargs):
    ...
    for pkg_name in locks:
        ...
        if all or (pkg_name in dpkgs and (not lock_ver or lock_ver == dpkgs[pkg_name])):
            if __opts__["test"]:
                comments.append("The following package would be unheld: {}{}".format(
                    pkg_name, "" if not dpkgs.get(pkg_name) else " (version = {})".format(lock_ver)))
            else:
                unhold_ret = __salt__["pkg.unhold"](name=name, pkgs=[pkg_name])
                if not unhold_ret.get(pkg_name, {}).get("result", False):
                    ret["result"] = False
